...
def isContainWindowDir(strPath):
    bRet = False
        
    lstDir = os.path.splitdrive(strPath)
    tmpDir = lstDir[1]
    
    if tmpDir.find("\\WINDOWS\\") == 0:
        bRet = True
...
if __name__ == "__main__":
    ...
    try:
        ...
        print("[I] Output Filename : "+".\\csv\\PD_MAL_CB_"+strCBVer+"_"+strMD5+".csv")
    except IOError:
    ...
    for curProc in jsonData['behavior']['processes']:
        ...
        BindHandleInfos[curProc['process_id']] = {}
    ...
    for curProc in jsonData['behavior']['processes']:
        ...
        for curCall in curProc['calls']:
            ...
            for curFeatAPI in lstFeatAPI:
                iFeatIdx = iFeatIdx + 1
                try:
                    curFeatAPIIdx = curFeatAPI.index(curCall['api'])                                        
                except ValueError:
                    continue
                ...
                if iFeatIdx == 1:
                    ...
                    if curCall['arguments'][0]['value'] != "0xffffffff":
                        ...
                        try:
                            ...
                            curWork['Arg2'] = "TargetProcHandle: "+curCall['arguments'][0]['value']
                    ...
                    if curCall['api'] == "URLDownloadToFileW" != -1:
                        ...
                        curInetHandleTable[str(hex(int(curCall['arguments'][0]['value'], 16) + 4), )] = curCall['arguments'][1]['value']+":"+curCall['arguments'][2]['value']
                        
                    elif curCall['api'].find("HttpOpenRequest") != -1:
                        # HttpOpenRequestA, HttpOpenRequestW Same Arguments
                        # Get Server Info from Table
                        try:
                            tmpServer = curInetHandleTable[str(hex(int(curCall['arguments'][0]['value'])))]
                        except:
                            continue
                    ...
                    # When Other Process Create Event occur                    
                    if curCall['api'] == 'CreateProcessInternalW':
                        if ((curCall['status'] == "SUCCESS") | (curCall['status'] == True)) & (curCall['arguments'][1]['value'] != ""):
                            tmpProcPath = curCall['arguments'][1]['value'].split()[0]
                            
                            if (isSystemDir(os.path.dirname(tmpProcPath)) & (find_fake.isSysdirExe(os.path.basename(tmpProcPath)) == False)) | (isWindowDir(os.path.dirname(tmpProcPath)) & (find_fake.isWindirExe(os.path.basename(tmpProcPath)) == False)):
                                curWork['Arg1'] = "STATUS: "+str(curCall['status'])
                        ...
                        if (curCall['status'] == "SUCCESS") | (curCall['status'] == True):
                            tmpProcPath = curCall['arguments'][0]['value']
                            if (isSystemDir(os.path.dirname(tmpProcPath)) & (find_fake.isSysdirExe(os.path.basename(tmpProcPath)) == False)) | (isWindowDir(os.path.dirname(tmpProcPath)) & (find_fake.isWindirExe(os.path.basename(tmpProcPath)) == False)):
                                curWork['Arg1'] = "STATUS: "+str(curCall['status'])
                    ...
                    if curCall['api'] == 'CreateProcessInternalW':
                        if ((curCall['status'] == "SUCCESS") | (curCall['status'] == True)) & (curCall['arguments'][1]['value'] != ""):
                            ...
                            if (isSystemDir(os.path.dirname(tmpProcPath)) | isWindowDir(os.path.dirname(tmpProcPath))) & find_fake.findFakeExeName(os.path.basename(tmpProcPath)):
                                curWork['Arg1'] = "STATUS: "+str(curCall['status'])
                    ...
                    tmpDllName = curCall['arguments'][1]['value']
                    
                    if find_fake.findFakeDllName(os.path.basename(tmpDllName)):
                        curWork['Arg1'] = "STATUS: "+str(curCall['status'])
